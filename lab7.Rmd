...
babynames %>%
  group_by(name) %>%
  mutate(
    girl_count = ifelse(sex == "F", 1, 0),
    boy_count = ifelse(sex == "M", 1, 0)) %>%
  mutate(
    total_count = sum(girl_count) + sum(boy_count)) %>%
  select(name, total_count) %>%
  filter(total_count == 276) #this wouldn't run
...
```{r}
babynames %>%
  filter(year %in% 1990:1999) %>% #this is where I got stuck
  group_by(name) %>% 
  mutate(
    popularity = sum(prop)) %>%
  ...
  head(n = 10)
#this is when babynames stopped existing
```
---
...
babynames %>%
  ...
  slice_max(
    order_by = earliest_year)

babynames
#total girls: 1938009
#total boys: 2027135
#total: 3965144
#n must be more than: 39, 651
```
...
babynames %>%
  group_by(year) %>%
  mutate(
    diversity = unique(name)) 

diversity_plot <- ggplot(babynames, aes(x = diversity, y = year)) %>%
  geom_line(stat = "identity", aes(y = diversity))
